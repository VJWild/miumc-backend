...
// =========================================================================
// RUTAS DE GESTIÓN DE INSCRIPCIONES (NUEVO)
// =========================================================================
...
app.get('/api/enrollments/:studentCode', async (req, res) => {
    try {
        ...
        const enrollments = rows.map(row => ({
            ...row,
            ...row.schedule_data // Extrae day, startTime, endTime, color, room
        }));
...
// 🌟 ACTUALIZADO: RECIBE TELÉFONO (PHONE) EN EL ONBOARDING
app.post('/api/onboarding/complete', async (req, res) => {
    const { studentCode, fullName, age, birthDate, phone, mencionId, approvedSubjects } = req.body;
    try {
        // Actualizar perfil del usuario con TELÉFONO
        await pool.query(
            'UPDATE users SET full_name = ?, age = ?, birth_date = ?, phone = ?, specialization_id = ? WHERE student_code = ?',
            [fullName, age, birthDate, phone || 'Sin teléfono', mencionId, studentCode]
        );
...
// 6. Listado global de usuarios con Carrera y Teléfono (simulado)
app.get('/api/admin/users', async (req, res) => {
    try {
        const query = `
            SELECT
                u.id, u.student_code, u.full_name, u.email, u.role,
                c.name as career_name,
                s.name as mencion_name,
                'Sin asignar' as phone -- Reemplazar por u.phone cuando se agregue a la BD
            FROM users u
                     LEFT JOIN careers c ON u.career_id = c.id
                     LEFT JOIN specializations s ON u.specialization_id = s.id
...
// 7. Editar datos de usuario (Incluye cambio de ROL y Carrera)
app.put('/api/admin/users/:id', async (req, res) => {
    const { id } = req.params;
    const { full_name, email, role, career_id, specialization_id } = req.body;
    try {
        const query = `
            UPDATE users
            SET full_name = ?, email = ?, role = ?, career_id = ?, specialization_id = ?
            WHERE id = ?
        `;
        await pool.query(query, [full_name, email, role, career_id || 1, specialization_id || 1, id]);
...
// 8. Eliminar usuario del sistema
...
// 9. Actualización masiva de récord académico (Transaccional)
app.post('/api/admin/update-records-bulk', async (req, res) => {
    ...
    try {
        ...
        // Limpiar el récord anterior
        await connection.query('DELETE FROM academic_records WHERE user_id = ?', [userId]);

        // Insertar el nuevo bloque de materias aprobadas
